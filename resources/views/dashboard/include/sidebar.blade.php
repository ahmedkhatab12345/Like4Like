...
<img class="avatar img-thumbnail rounded-circle shadow" src="{{ url('/') }}/images/dashboard/like4like.png" alt="">
...
<svg class="bi bi-hamburger" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
<path d="M1 0h14a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V1a1 1 0 0 1 1-1zM0 8h16a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1H0a1 1 0 0 0-1 1v2a1 1 0 0 0 1 1zm1 5h14a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1v-3a1 1 0 0 1 1-1z"/>
</svg>
...
<li class="collapsed">
    ...
    <li>
        <a class="ms-link text-warning" href="{{route('subscriptions.index')}}">
            مراجعة الاشتراكات
            <span class="badge rounded-pill bg-warning float-right ml-2">{{ App\Models\Subscription::where('status', 'pending')->count() }}</span>
    ...
    <li>
        <a class="ms-link text-success" href="{{route('accepted.subscription')}}">
            الاشتراكات المفعلة
            <span class="badge rounded-pill bg-success float-right ml-2">{{ App\Models\Withdrawal::where('status', 'active')->count() }}</span>
...
<span class="ms-2">الارباح</span>

<span class="arrow fa fa-angle-right ms-auto text-end"></span>
</a>

<ul class="sub-menu collapse" id="menu-Authentication">
<li><a class="ms-link" href="{{ route('earnings.index') }}">متابعه الاحصائيات</a></li>
</ul>
</li>
<li class="collapsed">
...
<ul class="sub-menu collapse" id="menu_pages">
    <li>
        <a class="ms-link text-warning" href="{{ route('withdrawals.index') }}">
            ادارة عمليات السحب
            <span class="badge rounded-pill bg-warning float-right ml-2">{{ App\Models\Withdrawal::where('status', 'pending')->count() }}</span>
        </a>
    </li>
    <li>
        <a class="ms-link text-success" href="{{ route('accepted.withdrawals') }}"> 
            العمليات المؤكدة
            <span class="badge rounded-pill bg-success float-right ml-2">{{ App\Models\Withdrawal::where('status', 'accept')->count() }}</span>
        </a>
    </li>
    <li>
        <a class="ms-link text-danger" href="{{ route('rejected.withdrawals') }}"> 
            العمليات المرفوضة
            <span class="badge rounded-pill bg-danger float-right ml-2">{{ App\Models\Withdrawal::where('status', 'rejected')->count() }}</span>
        </a>
    </li>
</ul>
